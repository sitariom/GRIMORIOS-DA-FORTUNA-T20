...
const DivinePointsPage: React.FC = () => {
  const { members, updateMember, bulkUpdateMembers, guildName, isLoading } = useGuild();
  ...
  const totalDivinePoints = activeMembers.reduce((sum, m) => sum + (m.divinePoints || 0), 0);
  const maxPoints = Math.max(...activeMembers.map(m => m.divinePoints || 0), 1);
  ...
  return (
    <div className="space-y-6">
      <header className="flex justify-between items-end mb-6 border-b-2 border-fantasy-gold/30 pb-4">
        ...
        <div className="flex items-center gap-4">
          {activeMembers.length > 0 && (
            <div className="flex items-center gap-2 bg-fantasy-gold/10 px-4 py-2 rounded-full border border-fantasy-gold/20">
              <Sparkles size={18} className="text-fantasy-gold" />
              <span className="transition-all duration-1000 font-bold text-fantasy-gold text-lg">{totalDivinePoints}</span>
              <span className="text-fantasy-wood/60 dark:text-fantasy-parchment/60 text-sm">PD Total</span>
            </div>
          )}
          <button 
            onClick={() => setShowRules(!showRules)}
            className="flex items-center gap-2 px-4 py-2 bg-[#2a1b14] hover:bg-[#3d2b1f] border border-fantasy-gold/30 rounded-lg text-fantasy-gold transition-colors text-sm font-bold shadow active:scale-95"
          >
      ...
      <div className="flex gap-4 mb-6">
        <button onClick={() => handleAddAll(1)} className="px-4 py-2 bg-blue-900/20 hover:bg-blue-900/40 text-blue-500 border border-blue-900/30 rounded-lg flex items-center gap-2 font-bold shadow transition-colors active:scale-95">
          <Plus size={18} /> Todos: +1 PD
        </button>
        <button onClick={() => handleAddAll(2)} className="px-4 py-2 bg-purple-900/20 hover:bg-purple-900/40 text-purple-400 border border-purple-900/30 rounded-lg flex items-center gap-2 font-bold shadow transition-colors active:scale-95">
          <Plus size={18} /> Todos: +2 PD
        </button>
        <button onClick={handleResetAll} className="px-4 py-2 bg-red-900/20 hover:bg-red-900/40 text-red-500 border border-red-900/30 rounded-lg flex items-center gap-2 font-bold shadow transition-colors ml-auto active:scale-95">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {activeMembers.length > 0 && totalDivinePoints === 0 && (
          <div className="col-span-full">
            <EmptyState icon={Sparkles} title="Nenhum ponto divino..." description="Os pontos divinos dos aventureiros aparecerão aqui." />
          </div>
        )}
        {activeMembers.length > 0 && totalDivinePoints > 0 && activeMembers.map((member, index) => {
          const pd = member.divinePoints || 0;
          const isMax = pd === maxPoints;
          return (
            <AnimatedCard key={member.id} delay={index * 80}>
              <div className={`parchment-card p-6 rounded-2xl shadow-xl border-2 border-[#3d2b1f] relative overflow-hidden group ${isMax ? 'shadow-[0_0_20px_rgba(212,175,55,0.2)]' : ''}`}>
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="bg-black/10 dark:bg-black/50 px-3 py-1 rounded-full flex items-center gap-1 shrink-0">
                    ...
                    <span className="font-bold text-fantasy-gold transition-all duration-1000">{pd} PD</span>
                  </div>
                </div>

                <div className="mb-4">
                  <div className="h-2 rounded-full bg-fantasy-gold/30">
                    <div
                      className="h-2 rounded-full bg-fantasy-gold transition-all duration-1000"
                      style={{ width: `${(pd / maxPoints) * 100}%` }}
                    />
                  </div>
                </div>
                ...
                <div className="flex justify-center items-center gap-6 mt-6">
                  <button 
                    ...
                    disabled={pd <= 0}
                    className="p-3 bg-red-900/10 hover:bg-red-900/30 text-red-600 disabled:opacity-30 disabled:hover:bg-red-900/10 rounded-full transition-colors active:scale-95"
                  >
                    <Minus size={24} />
                  </button>
                  ...
                  <button 
                    onClick={() => handleUpdate(member.id, pd, 1)}
                    className="p-3 bg-green-900/10 hover:bg-green-900/30 text-green-600 rounded-full transition-colors active:scale-95"
                  >
                    <Plus size={24} />
                  </button>
                </div>
