...
import { ArrowLeftRight, Scroll, Coins, AlertCircle, TrendingUp, Wallet } from 'lucide-react';
import AnimatedCard from '../components/AnimatedCard';
import EmptyState from '../components/EmptyState';
import { CardSkeleton } from '../components/LoadingSkeleton';

const FinancialPage: React.FC = () => {
  const { wallet, members, deposit, withdraw, convertWallet, notify, logs, isLoading } = useGuild();
    ...
    <div className="space-y-12 pb-20 font-serif">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          ...
          <AnimatedCard key={i} as="button" onClick={() => handleCoinClick(coin.code as CurrencyType)} delay={i * 100} className={`parchment-card p-10 rounded-[40px] border-b-[8px] ${coin.border} shadow-2xl text-left hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 group`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div className="parchment-card p-8 md:p-12 rounded-[60px] shadow-2xl border-4 border-fantasy-wood/10 dark:border-white/10">
          ...
          <div className="flex gap-4 md:gap-6 mb-12 bg-black/10 p-2.5 rounded-[32px] border border-fantasy-wood/10 dark:border-white/10">
            <button onClick={() => setOpType('deposit')} className={`flex-1 py-4 md:py-5 rounded-[24px] font-medieval uppercase tracking-widest text-base md:text-lg transition-all active:scale-95 ${opType === 'deposit' ? 'bg-emerald-800 text-white shadow-2xl scale-105' : 'text-fantasy-wood/40 dark:text-fantasy-parchment/40 hover:text-fantasy-wood'}`}>Entrada</button>
            <button onClick={() => setOpType('withdraw')} className={`flex-1 py-4 md:py-5 rounded-[24px] font-medieval uppercase tracking-widest text-base md:text-lg transition-all active:scale-95 ${opType === 'withdraw' ? 'bg-red-800 text-white shadow-2xl scale-105' : 'text-fantasy-wood/40 dark:text-fantasy-parchment/40 hover:text-fantasy-wood'}`}>Saída</button>
          </div>
          ...
          <form onSubmit={handleOperation} className="space-y-10">
            ...
            <button type="submit" className={`w-full py-8 md:py-10 rounded-[40px] font-medieval text-2xl uppercase tracking-[0.2em] shadow-2xl transition-all duration-300 border-b-8 active:border-b-0 active:translate-y-2 active:scale-95 hover:shadow-2xl hover:scale-[1.02] ${opType === 'deposit' ? 'bg-emerald-800 border-emerald-950 text-white shadow-[0_10px_40px_rgba(6,78,59,0.3)] hover:bg-emerald-700' : 'bg-red-800 border-red-950 text-white shadow-[0_10px_40px_rgba(153,27,27,0.3)] hover:bg-red-700'}`}>
          ...
          <form onSubmit={handleConversion} className="space-y-10">
             <div className="space-y-3">
                <div className="flex justify-between items-end ml-4 mr-2">
                    ...
                    <button type="button" onClick={setMaxConversion} className="text-[10px] font-black uppercase bg-fantasy-wood/10 dark:bg-white/10 hover:bg-fantasy-wood/20 px-3 py-1 rounded-lg text-fantasy-wood dark:text-fantasy-parchment transition-colors active:scale-95">
                        Máx: {wallet[convFrom]}
                    </button>
                </div>
                <input 
      ...
      {logs.length === 0 && (
        <div className="parchment-card p-8 md:p-12 rounded-[60px] shadow-2xl border-4 border-fantasy-wood/10 dark:border-white/10">
          <h3 className="font-medieval text-3xl md:text-4xl text-fantasy-wood dark:text-fantasy-parchment mb-8 border-b-4 border-fantasy-wood/10 dark:border-white/10 pb-8 flex items-center gap-6">
            <Scroll size={40} className="text-fantasy-gold"/> Registro do Cofre
          </h3>
          <EmptyState icon={Wallet} title="Nenhum registro financeiro..." description="As movimentações do cofre aparecerão aqui." />
        </div>
      )}
    </div>
  );
