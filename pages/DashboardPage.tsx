...
const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-12 pb-20 font-serif">
      <div className="relative h-[400px] w-full rounded-[60px] overflow-hidden border-8 border-[#3d2b1f] shadow-2xl group">
        {heroImage ? (
        ...
        <div className="absolute bottom-12 left-12">
          <div className="flex items-center gap-4 mb-4">
             ...
             <h2 className="text-fantasy-gold font-medieval text-2xl tracking-[0.2em] uppercase">Registros de {guildName}</h2>
          </div>
          <h1 className="text-6xl md:text-8xl font-medieval text-white text-glow-gold tracking-tight leading-none drop-shadow-2xl">
            Grimório da Fortuna
          </h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          ...
          <div key={i} className="parchment-card p-10 rounded-[40px] animate-scroll-unroll shadow-2xl border-b-[10px] border-fantasy-wood/20 dark:border-fantasy-gold/20">
             <div className="flex justify-between items-center mb-8">
                <div className={`p-5 ${kpi.bg} rounded-3xl ${kpi.color}`}><kpi.icon size={36}/></div>
                <div className="wax-seal w-12 h-12"></div>
             </div>
             <div className="text-xs font-black uppercase tracking-[0.3em] text-fantasy-wood/40 dark:text-fantasy-parchment/40 mb-2">{kpi.label}</div>
             <div className={`text-4xl font-medieval ${kpi.color}`}>{kpi.val}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="parchment-card p-12 rounded-[60px] shadow-2xl border-4 border-fantasy-wood/10 dark:border-white/10">
           <h3 className="font-medieval text-4xl text-fantasy-wood dark:text-fantasy-parchment mb-12 border-b-4 border-fantasy-wood/10 dark:border-white/10 pb-8 flex items-center gap-6">
             <Scroll size={40} className="text-fantasy-gold"/> Livro de Contas de Arton
           </h3>
              ...
              {[
                ...
                <div key={i} className="group">
                  <div className="flex justify-between items-end mb-4">
                    <span className="font-medieval text-2xl text-fantasy-wood dark:text-fantasy-parchment">{coin.label}</span>
                    <span className="font-medieval text-3xl text-fantasy-wood dark:text-fantasy-gold">{coin.val.toLocaleString()}</span>
                  </div>
                  <div className="h-6 w-full bg-black/10 dark:bg-white/5 rounded-full border-2 border-fantasy-wood/10 dark:border-white/10 p-1.5 shadow-inner">
                    <div 
        ...
        <div className="parchment-card p-12 rounded-[60px] shadow-2xl border-4 border-fantasy-wood/10 dark:border-white/10 flex flex-col">
           <h3 className="font-medieval text-4xl text-fantasy-wood dark:text-fantasy-parchment mb-12 border-b-4 border-fantasy-wood/10 dark:border-white/10 pb-8 flex items-center gap-6">
             <LandPlot size={40} className="text-fantasy-gold"/> Mapa de Domínios Reais
           </h3>
           <div className="flex-1 space-y-8 overflow-y-auto custom-scrollbar pr-4 max-h-[500px]">
              {domains.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-24 text-fantasy-wood/20 dark:text-white/10 italic">
                   ...
                   <p className="font-medieval text-3xl uppercase tracking-widest">Terras Incógnitas...</p>
                </div>
              ) : (
                domains.map((d) => (
                  <div key={d.id} className="p-8 bg-white/40 dark:bg-black/40 rounded-[32px] border-l-[12px] border-fantasy-gold shadow-lg hover:bg-fantasy-gold/10 transition-colors">
                    <div className="flex justify-between items-center mb-6">
                       <span className="font-medieval text-3xl text-fantasy-wood dark:text-fantasy-parchment">{d.name}</span>
                       <span className="text-xs font-black bg-fantasy-wood dark:bg-fantasy-gold text-fantasy-parchment dark:text-black px-5 py-2 rounded-full uppercase tracking-widest">Nível {d.level}</span>
                    </div>
                    <div className="flex gap-8 text-sm font-black text-fantasy-wood/70 dark:text-fantasy-parchment/60 uppercase tracking-widest">
                       <div className="flex items-center gap-2"><Castle size={18}/> {d.buildings.length} Obras</div>
                       <div className="flex items-center gap-2"><Sword size={18}/> {d.units.length} Tropas</div>
                    </div>
                  </div>
                ))
              )}
