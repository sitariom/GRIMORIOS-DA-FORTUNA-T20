...
const NPCsPage: React.FC = () => {
  ...
  const filteredNpcs = npcs.filter(n => n.name.toLowerCase().includes(searchTerm.toLowerCase()) || n.role.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="space-y-12 pb-20 font-serif">
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
        ...
        <div className="flex flex-wrap gap-4 w-full lg:w-auto">
          {npcs.length > 0 && (
            <button onClick={() => { if(confirm(`Confirmar pagamento total da folha: T$ ${totalPayroll}?`)) payAllNPCs(); }} className="flex-1 lg:flex-none bg-emerald-800 hover:bg-emerald-700 text-white px-8 py-5 rounded-[28px] flex items-center justify-center gap-3 font-medieval uppercase tracking-widest shadow-xl border-b-4 border-emerald-950 transition-all active:translate-y-1 active:border-b-0">
               <Coins size={20} /> Pagar Folha (T$ {totalPayroll})
            </button>
          )}
      ...
      <div className="parchment-card p-6 rounded-[32px] border-2 border-fantasy-wood/10 dark:border-white/10 flex flex-col sm:flex-row gap-6 shadow-xl">
          ...
          <div className="bg-black/5 dark:bg-black/20 px-8 py-4 rounded-2xl border border-fantasy-wood/10 dark:border-white/10 text-[10px] font-black uppercase text-fantasy-wood/60 dark:text-fantasy-parchment/40 flex items-center justify-center gap-3 tracking-widest">
              <Filter size={14}/> {filteredNpcs.length} Cadastros
          </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
          {npcs.length === 0 && (
              <div className="col-span-full py-20 text-center opacity-50">
                  ...
                  <p className="font-medieval text-3xl text-fantasy-wood dark:text-fantasy-parchment uppercase">Nenhum contrato ativo.</p>
              </div>
          )}
          {filteredNpcs.map((npc, idx) => (
              <div key={npc.id} className="parchment-card p-10 rounded-[48px] border-2 border-fantasy-wood/10 dark:border-white/10 shadow-2xl group animate-slide-up hover:border-fantasy-gold/50 transition-all" style={{ animationDelay: `${idx * 50}ms` }}>
